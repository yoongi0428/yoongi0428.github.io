...
**모든 리조트가 리조트 몰빵이거나 수중 환경 몰빵인건 아니다.** 리조트 주변은 라군이 발달했으나 가까운 스노쿨링 포인트나 관련 프로그램을 운영하는 곳도 있고, 아니면 정말 적당히 반반인 리조트도 있다. 그러니 조사할 때, 라군 or 수중환경 으로만 적기보다 그 "정도"를 표시하고 메모하는 것이 결정하는 데에 도움이 된다. (ex 라군:수중 = 7:3)
